...
## Install your packages and load your data

For this chapter we will need the following R packages: 
- `tidyverse`
- `stringr`
- `dplyr`
...
pkgs <- c("tidyverse", "stringr", "readr", "dplyr")
...
### 3. Standardise sex, smoking status, and education variables
...
cleaning <- cleaning %>%
  mutate(
    ...
    smoking_status_clean = case_when(
      ...
      tolower(smoking_status) == "occasional" ~ "Occasional",
      TRUE ~ NA_character_
    ),
...
cleaning <- cleaning %>%
  mutate(
    exposure_cat_clean = case_when(
      ...
      job_clean %in% c("office worker","teacher","doctor","nurse","construction worker", "security","cleaner","bartender","porter","farmer","driver") ~ "low",
      TRUE ~ "low"
    )
  )
...
FEV1 and FVC are recorded in millilitres in the raw data, so we convert them to litres for analysis. The FEV1/FVC ratio is a key lung function indicator and values below normal suggest obstruction. Dring collection of this data some errors may have occured, so it is important we remove any outliers at this stage that could bias our analyses later on.
